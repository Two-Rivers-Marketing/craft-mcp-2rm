...
| Item | P | Status | Notes |
| --- | --- | --- | --- |
| Response verbosity — full SEOmatic MetaBundle in every entry response | high | **partial** | `create_entry`/`update_entry` return only written fields — **live-verified** 2026-08-10. But the *list* tools were never covered and are worse, because the cost multiplies by N: measured live, `list_entries limit=25` returns **127KB, 79% of it the SEOmatic `seo` field**. `list_drafts` was fixed the same way (fields opt-in via `includeFields`, 92% smaller) but `list_entries`/`get_entry` still dump everything. Changing `list_entries`' default is breaking for mbd — **open decision**, see [log/2026-08-10](../log/2026-08-10-new-tools-and-live-verification.md#open-decision--list_entries-verbosity). Source: [KCMA field report](../raw/2026-07-28-kcma-build-tool-findings.md). |
