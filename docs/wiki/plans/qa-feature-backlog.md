...
| Item | P | Status | Notes |
| --- | --- | --- | --- |
| Response verbosity — full SEOmatic MetaBundle in every entry response | high | **done** | `create_entry`/`update_entry` now return compact responses: only written fields serialized. `get_entry`/`list_entries` still return all fields. Pending live verify. Source: [KCMA field report](../raw/2026-07-28-kcma-build-tool-findings.md). |
