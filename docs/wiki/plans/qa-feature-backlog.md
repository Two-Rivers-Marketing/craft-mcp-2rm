...
| `template.exists` wrong for child types + no multi-root awareness | high | **done** | Fixed: resolves via `View::resolveTemplate()` across all registered roots, tries nesting-appropriate paths, reports `servedBy` (`project` or `plugin`). Pending live verify on kcma.ddev.site. Source: [KCMA field report](../raw/2026-07-28-kcma-build-tool-findings.md). |
| `create_block_type` missing `topLevel`/`parentBlockTypes` params | med | **partial** | `topLevel` param added (default `true`). `parentBlockTypes` deferred — requires modifying+saving other block types, doable via separate calls. Source: [KCMA field report](../raw/2026-07-28-kcma-build-tool-findings.md). |
| `newFields` cannot create relation fields | med | **done** | Added `entries` and `users` types with `sources`/`maxRelations` options. Pending live verify. Source: [KCMA field report](../raw/2026-07-28-kcma-build-tool-findings.md). |
...
| Item | P | Status | Notes |
| --- | --- | --- | --- |
| Response verbosity — full SEOmatic MetaBundle in every entry response | high | **done** (v1.7.0) | Closed in three passes: `create_entry`/`update_entry` return only written fields (v1.5.1); `list_drafts` fields opt-in (v1.6.1); `list_entries` fields opt-in via `includeFields` (v1.7.0, **breaking**) — live-measured **127,605 → 11,448 bytes at `limit=25`, 92% smaller**. `get_entry` still returns all fields by design (single element, ~1KB). Lesson: the first fix was scoped to the two tools this report named rather than the shared serializer, so each new listing tool re-inherited the bug. See [log/2026-08-11](../log/2026-08-11-land-get-form-fix-and-list-entries-verbosity.md). Source: [KCMA field report](../raw/2026-07-28-kcma-build-tool-findings.md). |
...
| No `delete_entry` tool | med | **done** | Added `delete_entry` with `dryRun` support and `dangerous: true`. Hard delete. Pending live verify. Source: [KCMA field report](../raw/2026-07-28-kcma-build-tool-findings.md). |
| Date fields shift silently on write | high | **done** | All date serialization now uses ISO-8601 with timezone offset (`format('c')`), both in `Serializer` and `EntryTools`. Input handling is Craft's — the fix ensures the output no longer hides the zone. Pending live verify. Source: [KCMA field report](../raw/2026-07-28-kcma-build-tool-findings.md). |
...
| `tinker` security error doesn't name the blocked pattern | low | **done** | Error now names the matched construct (e.g. `copy(`) and hints at the right tool for `copy()` specifically. Source: [KCMA field report](../raw/2026-07-28-kcma-build-tool-findings.md). |
